...
void print_object_tree(XRef *xref, Object *obj, int level,int trailer) {
	...
	// need to work out why the trailer object causes this to crash.
	// nned explicit test for trailer object and skip this line
...
*/
		...
		printf("\n");
		print_object_tree(xref,&ref,level,0);
	} else {
			if (obj->isDict()) {
...
int main(int argc, char *argv[])
{
   ...
   //fprintf(stderr, "xpdf library version %s\n", xpdfVersion);
	...
	if (objnum > 0) {
		...
		print_object_tree(xref,trailer,0,1);
